...
test('double payment confirm is idempotent: no duplicate financial order or invoice', function (): void {
    ...
    expect(fn () => $confirmHandler(new ConfirmCheckoutPaymentCommand(paymentId: $paymentId, providerPaymentId: $providerId)))
        ->toThrow(\App\Modules\Shared\Domain\Exceptions\PaymentAlreadyProcessedException::class);
